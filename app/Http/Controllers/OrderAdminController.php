...
class OrderAdminController extends Controller
{
    ...
    public function bookShipping(Request $request, Preorder $order)
    {
        ...
        if ($courierSource === 'myparcelasia') {
            $shipping = new \App\Services\ShippingService();
            $res = $shipping->bookShipmentMyParcel($order, $request->all());
            if ($res['success']) {
                if (!empty($res['awb'])) {
                ...
                return redirect()->route('admin.orders.show', $order)->with('status', $res['message']);
            }
        ...
        $shipping = new \App\Services\ShippingService();
        $res = $shipping->bookShipment($order, $orderData);
        if ($res['success']) {
            if (!empty($res['awb'])) {
                $order->tracking_number = $res['awb'];
                $order->shipping_status = 'shipped';
    ...
    public function refreshTracking(Preorder $order, \App\Services\EasyParcelService $easyParcel)
    {
        if (empty($order->tracking_number)) {
            return back()->with('error', 'No tracking number found for this order');
        }
        try {
            $isMyParcel = !empty($order->shipping_courier_name) && stripos($order->shipping_courier_name, 'myparcel') !== false;
            $status = null;
            if ($isMyParcel) {
                $mpa = new \App\Services\MyParcelAsiaService();
                $trace = $mpa->trace(['tracking' => $order->tracking_number]);
                if (!empty($trace['status'])) {
                    $data = $trace['data'] ?? [];
                    $status = is_array($data) ? ($data['status'] ?? ($data['current_status'] ?? null)) : null;
                }
                if (!$status) {
                    $statResp = $mpa->getShipmentStatuses(['tracking_no' => $order->tracking_number]);

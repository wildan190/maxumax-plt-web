...
class PaymentController extends Controller
{
     ...
     */
    public function createCheckoutSession(\App\Http\Requests\Payment\CreateCheckoutSessionRequest $request)
    {
    ...
    /**
     * Create Stripe checkout session for a single preorder product
     */
    public function createPreorderCheckoutSession(\App\Http\Requests\Preorder\StorePreorderRequest $request)
    {
        $data = $request->validated();
        $data['address'] = $this->orderService->formatAddress($data);
    ...
    /**
     * Handle successful payment for preorder
     */
    public function preorderSuccess(Request $request)
    {
        ...
        if ($existingOrder) {
            ...
            return redirect()->route($existingOrder->product->available_for_preorder ? 'preorder.thankyou' : 'order.thankyou', ['uuid' => $existingOrder->uuid]);
        }
        ...
        try {
            ...
            session()->forget('stripe_preorder_checkout');
            $firstOrder = $orders[0];
            return redirect()->route($firstOrder->product->available_for_preorder ? 'preorder.thankyou' : 'order.thankyou', ['uuid' => $firstOrder->uuid]);
    ...
    /**
     * Handle cancelled preorder payment
     */

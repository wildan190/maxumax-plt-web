                    ...
                    <div style="padding: 1.5rem; border-top: 1px solid #e5e7eb; background: #f9fafb;">
                        ...
                        @if($order->refund_status === 'pending')
                            <div style="background: #fef3c7; border: 1px solid #fbbf24; border-radius: 0.5rem; padding: 1rem; margin-bottom: 1rem;">
                                ...
                                <p style="color: #78350f; font-size: 0.875rem; margin: 0 0 0.5rem 0;">Amount: {{ $order->currency }} {{ number_format($order->refund_amount, 2) }}</p>
                                @if($order->refund_reason)
                                    <p style="color: #78350f; font-size: 0.875rem; margin: 0 0 1rem 0;">Reason: {{ $order->refund_reason }}</p>
                                @endif
                                <div style="display: flex; gap: 0.5rem;">
                                    <form action="{{ route('admin.orders.approveRefund', $order) }}" method="POST" style="display: inline;" class="js-confirm" data-title="Approve refund?" data-text="Refund akan diproses melalui Stripe.">
                                        @csrf
                                    ...
                                    <form action="{{ route('admin.orders.rejectRefund', $order) }}" method="POST" style="display: inline;" class="js-confirm" data-title="Reject refund request?" data-text="Refund request akan ditolak.">
                                        @csrf
                        ...
                        @elseif($order->refund_status === 'approved' || $order->refund_status === 'completed')
                            <div style="background: #d1fae5; border: 1px solid #22c55e; border-radius: 0.5rem; padding: 1rem; margin-bottom: 1rem;">
                                ...
                                <p style="color: #047857; font-size: 0.875rem; margin: 0;">Refund ID: {{ $order->stripe_refund_id ?? 'N/A' }}</p>
                            </div>
                        ...
                        @else
                            <form action="{{ route('admin.orders.requestRefund', $order) }}" method="POST" id="refundForm" style="display: none;">
                                ...
                                <div style="margin-bottom: 1rem;">
                                    ...
                                    <input type="number" name="refund_amount" step="0.01" min="0" max="{{ $order->total_amount }}" value="{{ $order->total_amount }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.95rem;">
                                </div>
                                <div style="margin-bottom: 1rem;">
                        ...
                        @if($order->shipping_status)
                            ...
                            @if($order->tracking_number)
                                <div style="margin-bottom: 1rem;">
                                    ...
                                    <p style="font-weight: 600; color: #111827; margin: 0; font-family: monospace;">{{ $order->tracking_number }}</p>
                                </div>
                            @endif
                        @endif

                        @if(!$order->shipping_status || $order->shipping_status === 'pending')
                            <form action="{{ route('admin.orders.markPacking', $order) }}" method="POST" style="display: inline;">
                                @csrf
                        ...
                        @elseif($order->shipping_status === 'packing')
                            <form action="{{ route('admin.orders.markShipped', $order) }}" method="POST" id="shippedForm" style="display: none; margin-top: 1rem;">
                                ...
                                <div style="margin-bottom: 1rem;">
                                    ...
                                    <input type="text" name="tracking_number" value="{{ old('tracking_number', $order->tracking_number) }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.95rem; font-family: monospace;" placeholder="e.g., JNE1234567890">
                                </div>
                                <div style="display: flex; gap: 0.5rem;">

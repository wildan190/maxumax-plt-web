...
<div>
    ...
    <div style="display: flex; justify-content: flex-end; margin-bottom: 1rem;">
        <form action="{{ route('dashboard') }}" method="GET" id="currencyForm">
            <select name="currency" onchange="document.getElementById('currencyForm').submit()" style="padding: 0.5rem; border-radius: 0.375rem; border: 1px solid var(--border-gray); background: var(--secondary); color: var(--text-primary); font-weight: 600;">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; margin-bottom: 2rem;">
        ...
        <div style="background: var(--secondary); border: 1px solid var(--border-gray); border-radius: 0.75rem; padding: 1.5rem; transition: all 0.3s ease;">
            ...
            <p style="font-size: 2.5rem; font-weight: 700; color: var(--primary); margin: 0;">{{ number_format($totalOrders) }}</p>
        ...
        <div style="background: var(--secondary); border: 1px solid var(--border-gray); border-radius: 0.75rem; padding: 1.5rem; transition: all 0.3s ease;">
            ...
            <p style="font-size: 2.5rem; font-weight: 700; color: var(--primary); margin: 0;">{{ $currencySymbol }} {{ number_format($totalRevenue, 0, ',', '.') }}</p>
        ...
        <div style="background: var(--secondary); border: 1px solid var(--border-gray); border-radius: 0.75rem; padding: 1.5rem; transition: all 0.3s ease;">
            ...
            <p style="font-size: 2.5rem; font-weight: 700; color: #ef4444; margin: 0;">{{ $currencySymbol }} {{ number_format($totalRefunded, 0, ',', '.') }}</p>
            <p style="font-size: 0.875rem; color: var(--text-light); margin: 0.5rem 0 0;">{{ $refundedOrdersCount }} refunded orders</p>
        </div>
        ...
        <div style="background: var(--secondary); border: 1px solid var(--border-gray); border-radius: 0.75rem; padding: 1.5rem; transition: all 0.3s ease;">
            ...
            <p style="font-size: 2.5rem; font-weight: 700; color: {{ $growth >= 0 ? '#22c55e' : '#ef4444' }}; margin: 0;">
    ...
    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem; margin-bottom: 2rem;">
        ...
        <div style="background: var(--secondary); border: 1px solid var(--border-gray); border-radius: 0.75rem; padding: 1.5rem;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                ...
                <select style="padding: 0.5rem; border-radius: 0.375rem; border: 1px solid var(--border-gray); background: var(--secondary); color: var(--text-primary);">
        ...
        <div style="background: var(--secondary); border: 1px solid var(--border-gray); border-radius: 0.75rem; padding: 1.5rem;">
            ...
            <div style="display: flex; flex-direction: column; gap: 1rem;">
                <!-- Task Item: Refund -->
                <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 0.75rem; border-bottom: 1px solid var(--border-gray);">
                    <div>
                        <p style="font-weight: 600; color: var(--primary); margin: 0;">Refund Requests</p>
                        <p style="font-size: 0.875rem; color: var(--text-light); margin: 0.25rem 0 0;">Waiting for approval</p>
                    </div>
                    @if($pendingRefunds > 0)
                        <span style="background: #ef4444; color: white; padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.875rem; font-weight: 600;">{{ $pendingRefunds }}</span>
                    @else
                        <span style="background: #22c55e; color: white; padding: 0.25rem 0.5rem; border-radius: 50%; font-size: 0.875rem;">✓</span>
                    @endif
                </div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 0.75rem; border-bottom: 1px solid var(--border-gray);">
                    ...
                    @if($ordersToPack > 0)
                        <span style="background: #f59e0b; color: white; padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.875rem; font-weight: 600;">{{ $ordersToPack }}</span>
                    @else
                        <span style="background: #22c55e; color: white; padding: 0.25rem 0.5rem; border-radius: 50%; font-size: 0.875rem;">✓</span>
                    @endif
                </div>
    ...
    <div style="background: var(--secondary); border: 1px solid var(--border-gray); border-radius: 0.75rem; padding: 2rem;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            ...
            <a href="{{ route('admin.orders.index') }}" style="color: var(--primary); text-decoration: none; font-size: 0.9rem; font-weight: 600;">View All &rarr;</a>
        </div>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <div>
                                ...
                                <p style="color: var(--text-light); font-size: 0.85rem; margin: 0.25rem 0 0;">
                                    {{ $order->order_number }} • {{ $order->quantity }} pcs • {{ $order->currency }} {{ number_format($order->total_amount, 0, ',', '.') }}
                                </p>
                            </div>
                            <div style="text-align: right;">
                                ...
                                <p style="color: var(--text-light); font-size: 0.75rem; margin: 0.25rem 0 0;">{{ $order->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    </div>
                </div>

...
<div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div id="import-form" class="hidden w-full lg:w-auto animate-in fade-in slide-in-from-top-4 duration-300">
            <form action="{{ route('admin.products.import') }}" method="POST" enctype="multipart/form-data" class="flex items-center gap-3 p-2 bg-white rounded-2xl border border-slate-200 shadow-sm">
                @csrf
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" 
                                           class="sr-only peer" 
                                           {{ $p->add_to_homepage ? 'checked' : '' }}
                                           onchange="toggleHomepage(event, '{{ $p->uuid }}', this.checked)">
...
@push('scripts')
<script>
    async function toggleHomepage(event, productId, isChecked) {
        const checkbox = event.target;
        const originalState = !isChecked;

        try {
            const response = await fetch(`/admin/products/${productId}/toggle-homepage`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({ add_to_homepage: isChecked })
            });
